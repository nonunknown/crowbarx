...
namespace Crowbar
{
    public class SourceQcFile52 : SourceQcFile
    {
        ...
        public SourceQcFile52(StreamWriter outputFileStream, string outputPathFileName, SourceMdlFileData52 mdlFileData, SourceVtxFileData07 vtxFileData, SourcePhyFileData phyFileData, string modelName)
        {
        ...
        public SourceQcFile52(StreamWriter outputFileStream, string outputPathFileName, SourceMdlFileData52 mdlFileData, string modelName)
        {
            theOutputFileStreamWriter = outputFileStream;
            theMdlFileData = mdlFileData;
            theModelName = modelName;
        ...
        public SourceQcFile52(StreamWriter outputFileStream, SourceMdlFileData52 mdlFileData, string modelName)
        {
            theOutputFileStreamWriter = outputFileStream;
            theMdlFileData = mdlFileData;
            theModelName = modelName;
                ...
                if (aBodyPart.theModels is object && aBodyPart.theModels.Count > 0)
                {
                    ...
                    if (aModel.theEyeballs is object && aModel.theEyeballs.Count > 0)
                    {
                        ...
                        for (int eyeballIndex = 0, loopTo = aModel.theEyeballs.Count - 1; eyeballIndex <= loopTo; eyeballIndex++)
                        {
                            ...
                            frameIndex = 0;
                            for (int flexFrameIndex = 1, loopTo1 = theMdlFileData.theFlexFrames.Count - 1; flexFrameIndex <= loopTo1; flexFrameIndex++)
                            {
                                aFlexFrame = theMdlFileData.theFlexFrames[flexFrameIndex];
                            ...
                            // NOTE: Start at index 1 because defaultflex frame is at index 0.
                            frameIndex = 0;
                            for (int flexFrameIndex = 1, loopTo2 = theMdlFileData.theFlexFrames.Count - 1; flexFrameIndex <= loopTo2; flexFrameIndex++)
                            {
                                aFlexFrame = theMdlFileData.theFlexFrames[flexFrameIndex];
                                if ((aFlexFrame.flexName ?? "") == (eyelidName ?? ""))
                                {
                            ...
                            // NOTE: Start at index 1 because defaultflex frame is at index 0.
                            frameIndex = 0;
                            for (int flexFrameIndex = 1, loopTo3 = theMdlFileData.theFlexFrames.Count - 1; flexFrameIndex <= loopTo3; flexFrameIndex++)
                            {
                                aFlexFrame = theMdlFileData.theFlexFrames[flexFrameIndex];
                                if ((aFlexFrame.flexName ?? "") == (eyelidName ?? ""))
                                {
                            ...
                            // NOTE: Start at index 1 because defaultflex frame is at index 0.
                            frameIndex = 0;
                            for (int flexFrameIndex = 1, loopTo4 = theMdlFileData.theFlexFrames.Count - 1; flexFrameIndex <= loopTo4; flexFrameIndex++)
                            {
                                aFlexFrame = theMdlFileData.theFlexFrames[flexFrameIndex];
                                if ((aFlexFrame.flexName ?? "") == (eyelidName ?? ""))
                                {
            ...
            if (aSeqDesc.autoLayerCount > 0)
            {
                ...
                for (int j = 0, loopTo = aSeqDesc.theAutoLayers.Count - 1; j <= loopTo; j++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            SourceMdlAnimationDesc52 anAnimationDesc;
                            anAnimationDesc = theMdlFileData.theAnimationDescs[aSeqDesc.theAnimDescIndexes[0]];
                            influenceStart = (layer.influenceStart * (anAnimationDesc.frameCount - 1)).ToString("0", Program.TheApp.InternalNumberFormat);
                            influencePeak = (layer.influencePeak * (anAnimationDesc.frameCount - 1)).ToString("0", Program.TheApp.InternalNumberFormat);
                            influenceTail = (layer.influenceTail * (anAnimationDesc.frameCount - 1)).ToString("0", Program.TheApp.InternalNumberFormat);
                            influenceEnd = (layer.influenceEnd * (anAnimationDesc.frameCount - 1)).ToString("0", Program.TheApp.InternalNumberFormat);
                        }
                        else
                        {
                            influenceStart = layer.influenceStart.ToString("0.######", Program.TheApp.InternalNumberFormat);
                            influencePeak = layer.influencePeak.ToString("0.######", Program.TheApp.InternalNumberFormat);
                            influenceTail = layer.influenceTail.ToString("0.######", Program.TheApp.InternalNumberFormat);
                            influenceEnd = layer.influenceEnd.ToString("0.######", Program.TheApp.InternalNumberFormat);
                        }
