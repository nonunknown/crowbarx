    ...
    public class SourceQcFile37 : SourceQcFile
    {
        ...
        public void WriteModelCommand()
        {
            ...
            if (theMdlFileData.theBodyParts is object && theMdlFileData.theBodyParts.Count > 0)
            {
                ...
                aBodyModel.theSmdFileNames[0] = SourceFileNamesModule.CreateBodyGroupSmdFileName(aBodyModel.theSmdFileNames[0], 0, 0, 0, theModelName, Conversions.ToString(theMdlFileData.theBodyParts[0].theModels[0].name));
                if (Program.TheApp.Settings.DecompileQcUseMixedCaseForKeywordsIsChecked)
                {
                    line = "$Model ";
                }
                ...
                if (aBodyPart.theModels is object && aBodyPart.theModels.Count > 0)
                {
                    ...
                    if (aModel.theEyeballs is object && aModel.theEyeballs.Count > 0)
                    {
                        ...
                        for (int eyeballIndex = 0, loopTo = aModel.theEyeballs.Count - 1; eyeballIndex <= loopTo; eyeballIndex++)
                        {
                            ...
                            {
                                irisTextureName = theMdlFileData.theTextures[anEyeball.irisMaterial].theFileName;
                            }
        ...
        public void WriteIncludeModelCommands()
        {
            ...
            if (theMdlFileData.theIncludeModelFileNames is object)
            {
                line = "";
                theOutputFileStreamWriter.WriteLine(line);
                for (int i = 0, loopTo = theMdlFileData.theIncludeModelFileNames.Count - 1; i <= loopTo; i++)
                {
                    string anIncludeModelFileName;
                    anIncludeModelFileName = theMdlFileData.theIncludeModelFileNames[i];
        ...
        private void FillInWeightLists()
        {
            ...
            {
                SourceMdlSequenceDesc37 aSeqDesc;
        ...
        private void WriteSequenceOrDeclareSequenceCommand()
        {
            ...
            if (theMdlFileData.theSequenceDescs is object)
            {
                for (int i = 0, loopTo = theMdlFileData.theSequenceDescs.Count - 1; i <= loopTo; i++)
                {
                    SourceMdlSequenceDesc37 aSequenceDesc;
        ...
        private void WriteSequenceOptions(SourceMdlSequenceDesc37 aSequenceDesc)
        {
            ...
            SourceMdlAnimationDesc37 firstAnimDesc;
        ...
        private void WriteCmdListOptions(SourceMdlSequenceDesc37 aSequenceDesc, SourceMdlAnimationDesc37 anAnimationDesc, SourceMdlAnimationDesc37 impliedAnimDesc)
        {
            ...
            if (anAnimationDesc.theIkRules is object)
            {
                ...
                foreach (SourceMdlIkRule37 anIkRule in anAnimationDesc.theIkRules)
                {
                    ...
                    line += " contact ";
                    line += tempValue.ToString("0.##", Program.TheApp.InternalNumberFormat);
                    line += " pad ";
            ...
            if (aSequenceDesc is null && anAnimationDesc.theMovements is object)
            {
            ...
            try
            {
                if (theMdlFileData.theIkChains is object)
                {
                    ...
                    for (int i = 0, loopTo = theMdlFileData.theIkChains.Count - 1; i <= loopTo; i++)
                    {
                        ...
                        offsetX = Math.Round(theMdlFileData.theIkChains[i].theLinks[0].contact.x, 3);
                        offsetY = Math.Round(theMdlFileData.theIkChains[i].theLinks[0].contact.y, 3);
                        offsetZ = Math.Round(theMdlFileData.theIkChains[i].theLinks[0].contact.z, 3);
                        if (Program.TheApp.Settings.DecompileQcUseMixedCaseForKeywordsIsChecked)
                        {
                            line = "$IKChain \"";
                        }
