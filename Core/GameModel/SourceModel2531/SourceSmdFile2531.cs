...
namespace Crowbar
{
    public class SourceSmdFile2531
    {
        ...
        public void WriteSkeletonSection()
        {
            ...
            for (int boneIndex = 0, loopTo = theMdlFileData.theBones.Count - 1; boneIndex <= loopTo; boneIndex++)
            {
                rotation = MathModule.ToEulerAngles(theMdlFileData.theBones[boneIndex].rotation);
                line = "    ";
                line += boneIndex.ToString(Program.TheApp.InternalNumberFormat);
                line += " ";
        ...
        public void WriteSkeletonSectionForAnimation(SourceMdlSequenceDescBase aSequenceDescBase, SourceMdlAnimationDescBase anAnimationDescBase)
        {
            ...
            SourceMdlSequenceDesc2531 aSequenceDesc;
        ...
        public void WriteTrianglesSection(int lodIndex, SourceVtxModel107 aVtxModel, SourceMdlModel2531 aBodyModel, int bodyPartVertexIndexStart)
        {
            ...
            line = "triangles";
            theOutputFileStreamWriter.WriteLine(line);
            SourceVtxModelLod107 aVtxLod;
            SourceVtxMesh107 aVtxMesh;
            SourceVtxStripGroup107 aStripGroup;
            SourceVtxStrip107 aStrip;
            int materialIndex;
            string materialName;
                    ...
                    for (int meshIndex = 0, loopTo = aVtxLod.theVtxMeshes.Count - 1; meshIndex <= loopTo; meshIndex++)
                    {
                        ...
                        if (aVtxMesh.theVtxStripGroups is object)
                        {
                            for (int groupIndex = 0, loopTo1 = aVtxMesh.theVtxStripGroups.Count - 1; groupIndex <= loopTo1; groupIndex++)
                            {
                                ...
                                if (aStripGroup.theVtxStrips is object && aStripGroup.theVtxIndexes is object && (aStripGroup.theVtxVertexesForStaticProp is object || aStripGroup.theVtxVertexes is object))
                                {
                                    for (int vtxStripIndex = 0, loopTo2 = aStripGroup.theVtxStrips.Count - 1; vtxStripIndex <= loopTo2; vtxStripIndex++)
                                    {
                                        aStrip = aStripGroup.theVtxStrips[vtxStripIndex];
                                        for (int vtxIndexIndex = 0, loopTo3 = aStrip.indexCount - 3; vtxIndexIndex <= loopTo3; vtxIndexIndex += 3)
                                        {
                                            materialLine = materialName;
                                            vertex1Line = GetVertexLine(aBodyModel, aStripGroup, vtxIndexIndex + aStrip.indexMeshIndex, lodIndex, meshVertexIndexStart, bodyPartVertexIndexStart);
                                            vertex2Line = GetVertexLine(aBodyModel, aStripGroup, vtxIndexIndex + aStrip.indexMeshIndex + 2, lodIndex, meshVertexIndexStart, bodyPartVertexIndexStart);
                                            vertex3Line = GetVertexLine(aBodyModel, aStripGroup, vtxIndexIndex + aStrip.indexMeshIndex + 1, lodIndex, meshVertexIndexStart, bodyPartVertexIndexStart);
        ...
        private string GetVertexLine(SourceMdlModel2531 aBodyModel, SourceVtxStripGroup107 aStripGroup, int aVtxIndexIndex, int lodIndex, int meshVertexIndexStart, int bodyPartVertexIndexStart)
        {
            ...
            int vertexIndex;
            var boneIndexes = new List<int>(4);
            var position = new SourceVector();
            var normal = new SourceVector();
            var texCoordU = default(double);
            var texCoordV = default(double);
            ...
            try
            {
                ...
                {
                    for (int i = 0, loopTo = aBodyModel.theVertexesType0[vertexIndex].boneIndex.Length - 1; i <= loopTo; i++)
                    {
                        weights.Add(aBodyModel.theVertexesType0[vertexIndex].weight[i] / 255d);
                        if (weights[i] > 0d)
                        {
                ...
                else if (aBodyModel.vertexListType == 1)
                {
                    ...
                    // NOTE: This seems to work for many models, but seems to be wrong for:  
                    // models\character\npc\common\freshcorpses\femalefreshcorpse\femalefreshcorpse.mdl
                    position.x = aBodyModel.theVertexesType1[vertexIndex].positionX / 65535d * (theMdlFileData.hullMaxPosition.x - theMdlFileData.theBodyParts[0].theModels[0].unknown01[0]);
                    position.y = aBodyModel.theVertexesType1[vertexIndex].positionY / 65535d * (theMdlFileData.hullMaxPosition.y - theMdlFileData.theBodyParts[0].theModels[0].unknown01[1]);
                    position.z = aBodyModel.theVertexesType1[vertexIndex].positionZ / 65535d * (theMdlFileData.hullMaxPosition.z - theMdlFileData.theBodyParts[0].theModels[0].unknown01[2]);
                    ...
                    // TODO: Verify these.
                    normal.x = aBodyModel.theVertexesType1[vertexIndex].normalX / 255d;
                    normal.y = aBodyModel.theVertexesType1[vertexIndex].normalY / 255d;
                    normal.z = aBodyModel.theVertexesType1[vertexIndex].normalZ / 255d;

                    // These are correct.
                    texCoordU = aBodyModel.theVertexesType1[vertexIndex].texCoordU / 255d;
                    texCoordV = aBodyModel.theVertexesType1[vertexIndex].texCoordV / 255d;
                }
                else if (aBodyModel.vertexListType == 2)
                {
                    ...
                    position.x = (aBodyModel.theVertexesType2[vertexIndex].positionX - 128) / 255d * (theMdlFileData.hullMaxPosition.x - theMdlFileData.theBodyParts[0].theModels[0].unknown01[0]);
                    position.y = (aBodyModel.theVertexesType2[vertexIndex].positionY - 128) / 255d * (theMdlFileData.hullMaxPosition.y - theMdlFileData.theBodyParts[0].theModels[0].unknown01[1]);
                    position.z = aBodyModel.theVertexesType2[vertexIndex].positionZ / 255d * (theMdlFileData.hullMaxPosition.z - theMdlFileData.theBodyParts[0].theModels[0].unknown01[2]);
                    ...
                    // TODO: Verify these.
                    normal.x = aBodyModel.theVertexesType2[vertexIndex].normalX / 255d;
                    normal.y = aBodyModel.theVertexesType2[vertexIndex].normalY / 255d;
                    normal.z = aBodyModel.theVertexesType2[vertexIndex].normalZ / 255d;

                    // These are correct.
                    texCoordU = aBodyModel.theVertexesType2[vertexIndex].texCoordU / 255d;
                    texCoordV = aBodyModel.theVertexesType2[vertexIndex].texCoordV / 255d;
                }
                else
                {
                    int debug = 4242;
                ...
                if (texCoordU < 0d || texCoordU > 1d)
                {
                    texCoordU -= Math.Truncate(texCoordU);
                }

                if (texCoordV < 0d || texCoordV > 1d)
                {
                    texCoordV -= Math.Truncate(texCoordV);
                }

                line = "  ";
                line += boneIndexes[0].ToString(Program.TheApp.InternalNumberFormat);
                line += " ";
                line += position.x.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += position.y.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                ...
                line += " ";
                line += texCoordU.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += (1d - texCoordV).ToString("0.000000", Program.TheApp.InternalNumberFormat);
                if (aBodyModel.vertexListType == 0)
                {
                    line += " ";
                    line += boneIndexes.Count.ToString(Program.TheApp.InternalNumberFormat);
                    for (int i = 0, loopTo1 = boneIndexes.Count - 1; i <= loopTo1; i++)
                    {
                        line += " ";
                        line += boneIndexes[i].ToString(Program.TheApp.InternalNumberFormat);
                        line += " ";
                        line += weights[i].ToString("0.000000", Program.TheApp.InternalNumberFormat);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void CalcAnimation(SourceMdlSequenceDesc2531 aSequenceDesc, SourceMdlAnimationDesc2531 anAnimationDesc, int frameIndex)
        {
            double s;
            SourceMdlBone2531 aBone;
            SourceMdlAnimation2531 anAnimation;
            SourceVector rot;
            SourceVector pos;
            AnimationFrameLine aFrameLine;
            s = 0d;
        ...
        private SourceVector CalcBoneRotation(int frameIndex, double s, SourceMdlBone2531 aBone, SourceMdlAnimation2531 anAnimation, ref SourceQuaternion rotationQuat)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    rot.x = ExtractAnimValue(frameIndex, anAnimation.theRotationAnimationXValues, aBone.rotationScale.x, 0d);
                }
                ...
                else
                {
                    ...
                    rot.y = ExtractAnimValue(frameIndex, anAnimation.theRotationAnimationYValues, aBone.rotationScale.y, 0d);
                }
                ...
                else
                {
                    ...
                    rot.z = ExtractAnimValue(frameIndex, anAnimation.theRotationAnimationZValues, aBone.rotationScale.z, 0d);
                }
                ...
                else
                {
                    ...
                    rot.w = ExtractAnimValue(frameIndex, anAnimation.theRotationAnimationWValues, aBone.rotationScale.w, 0d);
                }
