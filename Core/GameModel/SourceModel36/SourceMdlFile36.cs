            ...
            if (theMdlFileData.localSequenceCount > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 0, loopTo = theMdlFileData.localSequenceCount - 1; i <= loopTo; i++)
                    {
                        ...
                        fileOffsetStart = theInputFileReader.BaseStream.Position;
                        var aSeqDesc = new SourceMdlSequenceDesc36();
                        ...
                        // Next
                        for (int rowIndex = 0, loopTo1 = SourceModule2531.MAXSTUDIOBLENDS - 1; rowIndex <= loopTo1; rowIndex++)
                        {
                            for (int columnIndex = 0, loopTo2 = SourceModule2531.MAXSTUDIOBLENDS - 1; columnIndex <= loopTo2; columnIndex++)
                        ...
                        for (int x = 0, loopTo3 = aSeqDesc.unused.Length - 1; x <= loopTo3; x++)
                            aSeqDesc.unused[x] = theInputFileReader.ReadInt32();

                        // NOTE: Not sure why these bytes were ever included; they are always zeroes.
                        inputFileStreamPosition = theInputFileReader.BaseStream.Position;
                        theInputFileReader.BaseStream.Seek(inputFileStreamPosition + 3584L, SeekOrigin.Begin);
            ...
            if (theMdlFileData.sequenceGroupCount > 0)
            {
                ...
                try
                {
                    ...
                    for (int sequenceGroupIndex = 0, loopTo = theMdlFileData.sequenceGroupCount - 1; sequenceGroupIndex <= loopTo; sequenceGroupIndex++)
                    {
                        ...
                        {
                            for (int x = 0, loopTo1 = aSequenceGroup.unknown.Length - 1; x <= loopTo1; x++)
            ...
            if (theMdlFileData.animationCount > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 0, loopTo = theMdlFileData.animationCount - 1; i <= loopTo; i++)
                    {
                        ...
                        // fileOffsetStart = Me.theInputFileReader.BaseStream.Position
                        var anAnimationDesc = new SourceMdlAnimationDesc36();
