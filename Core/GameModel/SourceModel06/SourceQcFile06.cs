...
namespace Crowbar
{
    public class SourceQcFile06
    {
        ...
        {
            string line = "";
            SourceMdlBoneController06 boneController;
            try
            {
                if (theMdlFileData.theBoneControllers is object)
                {
                    ...
                    for (int boneControllerIndex = 0, loopTo = theMdlFileData.theBoneControllers.Count - 1; boneControllerIndex <= loopTo; boneControllerIndex++)
                    {
                        ...
                        line += boneControllerIndex.ToString(Program.TheApp.InternalNumberFormat);
                        line += " \"";
        ...
        private void WriteSequenceOptions(SourceMdlSequenceDesc06 aSequenceDesc)
        {
            ...
            if (aSequenceDesc.theEvents is object)
            {
                ...
                for (int j = 0, loopTo1 = aSequenceDesc.theEvents.Count - 1; j <= loopTo1; j++)
                {
                    ...
                    line += aSequenceDesc.theEvents[j].eventType.ToString(Program.TheApp.InternalNumberFormat);
                    line += " ";
