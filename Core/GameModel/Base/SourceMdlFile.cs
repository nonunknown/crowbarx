...
namespace Crowbar
{
    public class SourceMdlFile
    {
        ...
        public virtual void ReadMdlHeader00()
        {
        }

        public virtual void ReadMdlHeader01()
        {
        }

        public virtual void ReadMdlHeader02()
        {
        }

        public virtual void ReadTextures()
        {
        }

        public virtual void ReadTexturePaths()
        {
        }
        ...
        public virtual void ReadFileForViewer(string inputPathFileName, SourceMdlFileData aMdlFileData)
        {
            ...
            try
            {
                ...
                if (inputFileStream is object)
                {
                    try
                    {
                        theInputFileReader = new BinaryReader(inputFileStream, Encoding.ASCII);
                    ...
                    {
                        if (theInputFileReader is object)
                        {
        ...
        public void WriteHeaderNameToFile(string inputPathFileName, string headerName)
        {
            ...
            try
            {
                ...
                if (inputFileStream is object)
                {
                    BinaryWriter inputFileWriter = null;
                    try
                    {
                        // NOTE: Important to set System.Text.Encoding.ASCII so that ReadChars() only reads in one byte per Char.
                        inputFileWriter = new BinaryWriter(inputFileStream, Encoding.ASCII);
                        if (theMdlFileData.version > 10)
                        {
                            inputFileWriter.BaseStream.Seek(0xCL, SeekOrigin.Begin);
                        }
                        else
                        {
                            inputFileWriter.BaseStream.Seek(0x8L, SeekOrigin.Begin);
                        }
        ...
        protected SourceMdlFileData theMdlFileData;
