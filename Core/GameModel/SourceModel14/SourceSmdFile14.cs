    ...
    public class SourceSmdFile14
    {
        ...
        public void WriteTrianglesSection(SourceMdlModel14 aBodyModel)
        {
            ...
            try
            {
                ...
                // Next
                // End If
                for (int aVertexIndexIndex = 0, loopTo = theMdlFileData.theIndexes.Count - 3; aVertexIndexIndex <= loopTo; aVertexIndexIndex += 3)
                {
                    materialLine = theMdlFileData.theTextures[0].theTextureName;
                    boneIndex = 0;
                    aVertexIndex = theMdlFileData.theIndexes[aVertexIndexIndex];
                    vertex1Line = GetVertexLine(boneIndex, theMdlFileData.theVertexes[aVertexIndex], theMdlFileData.theNormals[aVertexIndex], theMdlFileData.theUVs[aVertexIndex], aBodyModel.theWeightingHeaders[0], theMdlFileData.theWeightings[aVertexIndex]);
                    aVertexIndex = theMdlFileData.theIndexes[aVertexIndexIndex + 2];
                    vertex2Line = GetVertexLine(boneIndex, theMdlFileData.theVertexes[aVertexIndex], theMdlFileData.theNormals[aVertexIndex], theMdlFileData.theUVs[aVertexIndex], aBodyModel.theWeightingHeaders[0], theMdlFileData.theWeightings[aVertexIndex]);
                    aVertexIndex = theMdlFileData.theIndexes[aVertexIndexIndex + 1];
                    vertex3Line = GetVertexLine(boneIndex, theMdlFileData.theVertexes[aVertexIndex], theMdlFileData.theNormals[aVertexIndex], theMdlFileData.theUVs[aVertexIndex], aBodyModel.theWeightingHeaders[0], theMdlFileData.theWeightings[aVertexIndex]);
        ...
        private string GetVertexLine(int boneIndex, SourceVector position, SourceVector normal, SourceVector uv, SourceMdlWeightingHeader14 weightingHeader, SourceMdlWeighting14 weighting)
        {
            ...
            try
            {
                ...
                line += " ";
                line += uv.x.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += (-uv.y).ToString("0.000000", Program.TheApp.InternalNumberFormat);
                if (weighting.boneCount > 0)
                {
                    line += " ";
                    line += weighting.boneCount.ToString(Program.TheApp.InternalNumberFormat);
                    for (int x = 0, loopTo = weighting.boneCount - 1; x <= loopTo; x++)
                    {
                        line += " ";
                        line += weightingHeader.theWeightingBoneDatas[0].theWeightingBoneIndexes[weighting.bones[x]].ToString(Program.TheApp.InternalNumberFormat);
                        line += " ";
                        line += weighting.weights[x].ToString("0.000000", Program.TheApp.InternalNumberFormat);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void CalcAnimation(SourceMdlSequenceDesc10 aSequenceDesc, int blendIndex, int frameIndex)
        {
            ...
            SourceMdlAnimation10 anAnimation;
            SourceVector rot;
            SourceVector pos;
            AnimationFrameLine aFrameLine;
            s = 0d;
