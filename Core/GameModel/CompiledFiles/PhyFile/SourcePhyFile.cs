...
namespace Crowbar
{
    public class SourcePhyFile
    {
        ...
        public SourcePhyFile(BinaryReader phyFileReader, SourcePhyFileData phyFileData, long endOffset = 0L)
        {
        ...
        public void ReadSourceCollisionData()
        {
            long fileOffsetStart;
            long fileOffsetEnd;
            var ivpsId = new char[4];
            int triangleCount;
            int triangleIndex;
            List<int> vertices;
            long nextSolidDataStreamPosition;
            long phyDataStreamPosition;
            long faceDataStreamPosition;
            long vertexDataStreamPosition;
            ...
            for (int solidIndex = 0, loopTo = thePhyFileData.solidCount - 1; solidIndex <= loopTo; solidIndex++)
            {
                var collisionData = new SourcePhyCollisionData();
                ...
                while (theInputFileReader.BaseStream.Position < vertexDataStreamPosition)
                {
                    ...
                    // 05 00 
                    // de 7f 
                    for (int i = 0, loopTo1 = triangleCount - 1; i <= loopTo1; i++)
                    {
                        var phyTriangle = new SourcePhyFace();
                ...
                double w;
                List<SourcePhyVertex> faceSection0Vertices;
                faceSection0Vertices = collisionData.theFaceSections[0].theVertices;
                for (int i = 0, loopTo2 = vertices.Count - 1; i <= loopTo2; i++)
                {
                    var phyVertex = new SourcePhyVertex();
                ...
                for (int faceSectionIndex = 1, loopTo3 = collisionData.theFaceSections.Count - 1; faceSectionIndex <= loopTo3; faceSectionIndex++)
                {
                    faceSection = collisionData.theFaceSections[faceSectionIndex];
                    for (int i = 0, loopTo4 = vertices.Count - 1; i <= loopTo4; i++)
                    {
                        var phyVertex = new SourcePhyVertex();
        ...
        public void ReadSourcePhysCollisionModels()
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    while (thereIsAValue)
                    {
                        ...
                        if (thereIsAValue)
                        {
                            if (key == "index")
                            {
                                aSourcePhysCollisionModel.theIndex = int.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "name")
                            {
                            ...
                            {
                                aSourcePhysCollisionModel.theMass = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            ...
                            {
                                aSourcePhysCollisionModel.theDamping = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            ...
                            {
                                aSourcePhysCollisionModel.theRotDamping = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            ...
                            else if (key == "drag")
                            {
                                ...
                                aSourcePhysCollisionModel.theDragCoefficient = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "rollingDrag")
                            {
                                aSourcePhysCollisionModel.theRollingDragCoefficientIsValid = true;
                                aSourcePhysCollisionModel.theRollingDragCoefficient = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "inertia")
                            {
                                aSourcePhysCollisionModel.theInertia = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            ...
                            {
                                aSourcePhysCollisionModel.theVolume = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "massbias")
                            {
                                aSourcePhysCollisionModel.theMassBiasIsValid = true;
                                aSourcePhysCollisionModel.theMassBias = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                        }
                    }
                ...
                maxCount = 0;
                for (int i = 0, loopTo1 = theInertiaToCountMap.Count - 1; i <= loopTo1; i++)
                {
                ...
                maxCount = 0;
                for (int i = 0, loopTo2 = theRotDampingToCountMap.Count - 1; i <= loopTo2; i++)
                {
        ...
        public void ReadSourcePhyRagdollConstraintDescs()
        {
            ...
            try
            {
                ...
                string value = "";
                long tempStreamOffset;
                SourcePhyRagdollConstraint aSourceRagdollConstraintDesc;
                ...
                do
                {
                    ...
                    tempStreamOffset = theInputFileReader.BaseStream.Position;
                    line = FileManager.ReadTextLine(theInputFileReader);
                    if (line is null || line != "ragdollconstraint {")
                    {
                        theInputFileReader.BaseStream.Seek(tempStreamOffset, SeekOrigin.Begin);
                        break;
                    }
                    ...
                    while (thereIsAValue)
                    {
                        ...
                        if (thereIsAValue)
                        {
                            if (key == "parent")
                            {
                                aSourceRagdollConstraintDesc.theParentIndex = int.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "child")
                            {
                                aSourceRagdollConstraintDesc.theChildIndex = int.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "xmin")
                            {
                                aSourceRagdollConstraintDesc.theXMin = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "xmax")
                            {
                                aSourceRagdollConstraintDesc.theXMax = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "xfriction")
                            {
                                aSourceRagdollConstraintDesc.theXFriction = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "ymin")
                            {
                                aSourceRagdollConstraintDesc.theYMin = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "ymax")
                            {
                                aSourceRagdollConstraintDesc.theYMax = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "yfriction")
                            {
                                aSourceRagdollConstraintDesc.theYFriction = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "zmin")
                            {
                                aSourceRagdollConstraintDesc.theZMin = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "zmax")
                            {
                                aSourceRagdollConstraintDesc.theZMax = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                            else if (key == "zfriction")
                            {
                                aSourceRagdollConstraintDesc.theZFriction = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                        }
                    }
        ...
        public void ReadSourcePhyCollisionRules()
        {
            ...
            try
            {
                ...
                string value = "";
                long tempStreamOffset;
                SourcePhyCollisionPair aSourcePhyCollisionPair;
                thePhyFileData.theSourcePhyCollisionPairs = new List<SourcePhyCollisionPair>();
                do
                {
                    tempStreamOffset = theInputFileReader.BaseStream.Position;
                    line = FileManager.ReadTextLine(theInputFileReader);
                    if (line is null || line != "collisionrules {")
                    {
                        theInputFileReader.BaseStream.Seek(tempStreamOffset, SeekOrigin.Begin);
                        break;
                    }
                        ...
                        if (thereIsAValue)
                        {
                            if (key == "collisionpair")
                            {
                                ...
                                if (tokens.Length == 2)
                                {
                                    ...
                                    aSourcePhyCollisionPair.obj0 = int.Parse(tokens[0], Program.TheApp.InternalNumberFormat);
                                    aSourcePhyCollisionPair.obj1 = int.Parse(tokens[1], Program.TheApp.InternalNumberFormat);
        ...
        public void ReadSourcePhyEditParamsSection()
        {
            ...
            try
            {
                ...
                do
                {
                    tempStreamOffset = theInputFileReader.BaseStream.Position;
                    line = FileManager.ReadTextLine(theInputFileReader);
                    if (line is null || line != "editparams {")
                    {
                        theInputFileReader.BaseStream.Seek(tempStreamOffset, SeekOrigin.Begin);
                        break;
                    }
                    ...
                    while (thereIsAValue)
                    {
                        ...
                        else if (thereIsAValue)
                        {
                            ...
                            {
                                thePhyFileData.theSourcePhyEditParamsSection.totalMass = float.Parse(value, Program.TheApp.InternalNumberFormat);
                            }
                        }
                    }
        ...
        public void ReadCollisionTextSection()
        {
            ...
            {
                long endOffset;
                if (thePhyEndOffset == 0L)
                {
                    endOffset = theInputFileReader.BaseStream.Length - 1L;
                }
                else
                {
        ...
        {
            var vertex = new SourceVector[4];
            var vector0 = new SourceVector();
            var vector1 = new SourceVector();
            var normalVector = new SourceVector();
