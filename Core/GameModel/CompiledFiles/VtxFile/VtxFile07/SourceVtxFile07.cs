...
namespace Crowbar
{
    public class SourceVtxFile07
    {
        ...
        public SourceVtxFile07(BinaryReader vtxFileReader, SourceVtxFileData07 vtxFileData, long vtxFileOffsetStart = 0L)
        {
        ...
        public void ReadSourceVtxBodyParts()
        {
            if (theVtxFileData.bodyPartCount > 0)
            {
                ...
                try
                {
                    ...
                    if (theSecondMeshWithStripGroups is null)
                    {
        ...
        public void ReadSourceVtxMaterialReplacementLists()
        {
            ...
            {
                long materialReplacementListInputFileStreamPosition;
                ...
                try
                {
                    ...
                    for (int i = 0, loopTo = theVtxFileData.lodCount - 1; i <= loopTo; i++)
                    {
                        materialReplacementListInputFileStreamPosition = theInputFileReader.BaseStream.Position;
                        var aMaterialReplacementList = new SourceVtxMaterialReplacementList07();
        ...
        private void ReadSourceVtxBodyParts_Internal()
        {
            ...
            try
            {
                ...
                for (int i = 0, loopTo = theVtxFileData.bodyPartCount - 1; i <= loopTo; i++)
                {
                    bodyPartInputFileStreamPosition = theInputFileReader.BaseStream.Position;
                    var aBodyPart = new SourceVtxBodyPart07();
            ...
            if (aBodyPart.modelCount > 0 && aBodyPart.modelOffset != 0)
            {
                ...
                try
                {
                    ...
                    for (int j = 0, loopTo = aBodyPart.modelCount - 1; j <= loopTo; j++)
                    {
                        ...
                        var aModel = new SourceVtxModel07();
        ...
        private void ReadSourceVtxModelLods(long modelInputFileStreamPosition, SourceVtxModel07 aModel)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aModel.lodCount - 1; j <= loopTo; j++)
                {
                    modelLodInputFileStreamPosition = theInputFileReader.BaseStream.Position;
                    var aModelLod = new SourceVtxModelLod07();
        ...
        private void ReadSourceVtxMeshes(long modelLodInputFileStreamPosition, SourceVtxModelLod07 aModelLod)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aModelLod.meshCount - 1; j <= loopTo; j++)
                {
                    meshInputFileStreamPosition = theInputFileReader.BaseStream.Position;
                    var aMesh = new SourceVtxMesh07();
                    ...
                    {
                        if (theFirstMeshWithStripGroups is null)
                        {
                        ...
                        // End If
                        // End If
                        else if (theSecondMeshWithStripGroups is null)
                        {
        ...
        private void AnalyzeVtxStripGroups(long meshInputFileStreamPosition, SourceVtxMesh07 aMesh)
        {
            try
            {
                ...
                for (int j = 0, loopTo = aMesh.stripGroupCount - 1; j <= loopTo; j++)
                {
                    var aStripGroup = new SourceVtxStripGroup07();
        ...
        private void ReadSourceVtxVertexes(long stripGroupInputFileStreamPosition, SourceVtxStripGroup07 aStripGroup)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.vertexCount - 1; j <= loopTo; j++)
                {
                    // modelInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
                    var aVertex = new SourceVtxVertex07();
                    for (int i = 0, loopTo1 = SourceConstants.MAX_NUM_BONES_PER_VERT - 1; i <= loopTo1; i++)
                    ...
                    for (int i = 0, loopTo2 = SourceConstants.MAX_NUM_BONES_PER_VERT - 1; i <= loopTo2; i++)
        ...
        private void ReadSourceVtxIndexes(long stripGroupInputFileStreamPosition, SourceVtxStripGroup07 aStripGroup)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.indexCount - 1; j <= loopTo; j++)
                    // modelInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
        ...
        {
            long stripInputFileStreamPosition;
            long inputFileStreamPosition;
            long fileOffsetStart;
            long fileOffsetEnd;
            // Dim fileOffsetStart2 As Long
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.stripCount - 1; j <= loopTo; j++)
                {
                    stripInputFileStreamPosition = theInputFileReader.BaseStream.Position;
                    var aStrip = new SourceVtxStrip07();
        ...
        private void ReadSourceVtxTopologyIndexes(long stripGroupInputFileStreamPosition, SourceVtxStripGroup07 aStripGroup)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.topologyIndexCount - 1; j <= loopTo; j++)
                {
                    // topologyInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
                    ushort topologyIndex;
                    topologyIndex = theInputFileReader.ReadUInt16();
        ...
        private void ReadSourceVtxBoneStateChanges(long stripInputFileStreamPosition, SourceVtxStrip07 aStrip)
        {
            ...
            if (boneStateChangeCount > 0 && aStrip.boneStateChangeOffset != 0)
            {
                ...
                try
                {
                    ...
                    for (int j = 0, loopTo = boneStateChangeCount - 1; j <= loopTo; j++)
                    {
                        // modelInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
                        // fileOffsetStart = Me.theInputFileReader.BaseStream.Position
                        var aBoneStateChange = new SourceVtxBoneStateChange07();
        ...
        private void ReadSourceVtxMaterialReplacements(long materialReplacementListInputFileStreamPosition, SourceVtxMaterialReplacementList07 aMaterialReplacementList)
        {
            ...
            {
                long materialReplacementInputFileStreamPosition;
                long inputFileStreamPosition;
                long fileOffsetStart;
                long fileOffsetEnd;
                ...
                try
                {
                    ...
                    for (int j = 0, loopTo = aMaterialReplacementList.replacementCount - 1; j <= loopTo; j++)
                    {
                        materialReplacementInputFileStreamPosition = theInputFileReader.BaseStream.Position;
                        var aMaterialReplacement = new SourceVtxMaterialReplacement07();
                        ...
                        else if (aMaterialReplacement.theName is null)
                        {
