    ...
    public class SourceVtxFile107
    {
            ...
            if (theVtxFileData.bodyPartCount > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 0, loopTo = theVtxFileData.bodyPartCount - 1; i <= loopTo; i++)
                    {
                        ...
                        fileOffsetStart = theInputFileReader.BaseStream.Position;
                        var aBodyPart = new SourceVtxBodyPart107();
            ...
            if (aBodyPart.modelCount > 0 && aBodyPart.modelOffset != 0)
            {
                ...
                try
                {
                    ...
                    for (int j = 0, loopTo = aBodyPart.modelCount - 1; j <= loopTo; j++)
                    {
                        ...
                        fileOffsetStart = theInputFileReader.BaseStream.Position;
                        var aModel = new SourceVtxModel107();
            ...
            if (aModel.lodCount > 0 && aModel.lodOffset != 0)
            {
                ...
                try
                {
                    ...
                    for (int j = 0, loopTo = aModel.lodCount - 1; j <= loopTo; j++)
                    {
                        ...
                        fileOffsetStart = theInputFileReader.BaseStream.Position;
                        var aModelLod = new SourceVtxModelLod107();
            ...
            if (aModelLod.meshCount > 0 && aModelLod.meshOffset != 0)
            {
                ...
                try
                {
                    ...
                    for (int j = 0, loopTo = aModelLod.meshCount - 1; j <= loopTo; j++)
                    {
                        ...
                        fileOffsetStart = theInputFileReader.BaseStream.Position;
                        var aMesh = new SourceVtxMesh107();
        ...
        private void ReadSourceVtxStripGroups(long meshInputFileStreamPosition, SourceVtxMesh107 aMesh)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aMesh.stripGroupCount - 1; j <= loopTo; j++)
                {
                    ...
                    fileOffsetStart = theInputFileReader.BaseStream.Position;
                    var aStripGroup = new SourceVtxStripGroup107();
        ...
        private void ReadSourceVtxVertexesForStaticProp(long stripGroupInputFileStreamPosition, SourceVtxStripGroup107 aStripGroup)
        {
            ...
            try
            {
                ...
                fileOffsetStart = theInputFileReader.BaseStream.Position;
                aStripGroup.theVtxVertexesForStaticProp = new List<ushort>(aStripGroup.vertexCount);
                for (int j = 0, loopTo = aStripGroup.vertexCount - 1; j <= loopTo; j++)
                {
                    ...
                    ushort aVertexMeshIndex;
                    aVertexMeshIndex = theInputFileReader.ReadUInt16();
        ...
        private void ReadSourceVtxVertexes(long stripGroupInputFileStreamPosition, SourceVtxStripGroup107 aStripGroup)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.vertexCount - 1; j <= loopTo; j++)
                {
                    // modelInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
                    // fileOffsetStart = Me.theInputFileReader.BaseStream.Position
                    var aVertex = new SourceVtxVertex107();
                    aVertex.unknown01 = theInputFileReader.ReadInt16();
                    for (int x = 0, loopTo1 = aVertex.boneIndex.Length - 1; x <= loopTo1; x++)
        ...
        private void ReadSourceVtxIndexes(long stripGroupInputFileStreamPosition, SourceVtxStripGroup107 aStripGroup)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.indexCount - 1; j <= loopTo; j++)
                {
                    ...
                    // fileOffsetStart = Me.theInputFileReader.BaseStream.Position
                    ushort anIndex;
                    anIndex = theInputFileReader.ReadUInt16();
        ...
        private void ReadSourceVtxStrips(long stripGroupInputFileStreamPosition, SourceVtxStripGroup107 aStripGroup)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStripGroup.stripCount - 1; j <= loopTo; j++)
                {
                    ...
                    // fileOffsetStart = Me.theInputFileReader.BaseStream.Position
                    var aStrip = new SourceVtxStrip107();
        ...
        private void ReadSourceVtxBoneStateChanges(long stripInputFileStreamPosition, SourceVtxStrip107 aStrip)
        {
            ...
            try
            {
                ...
                for (int j = 0, loopTo = aStrip.boneStateChangeCount - 1; j <= loopTo; j++)
                {
                    // modelInputFileStreamPosition = Me.theInputFileReader.BaseStream.Position
                    // fileOffsetStart = Me.theInputFileReader.BaseStream.Position
                    var aBoneStateChange = new SourceVtxBoneStateChange107();
