...
namespace Crowbar
{
    public class SourceMdlFile49
    {
        ...
        public void ReadMdlHeader02(string logDescription)
        {
            ...
            for (int x = 0, loopTo = theMdlFileData.reserved.Length - 1; x <= loopTo; x++)
        ...
        {
            long aimAtBoneInputFileStreamPosition;
            long fileOffsetStart;
            long fileOffsetEnd;
            try
            {
        ...
        private void ReadHitboxes(long hitboxOffsetInputFileStreamPosition, SourceMdlHitboxSet aHitboxSet)
        {
            if (aHitboxSet.hitboxCount > 0)
            {
                ...
                for (int j = 0, loopTo = aHitboxSet.hitboxCount - 1; j <= loopTo; j++)
                {
                    ...
                    for (int x = 0, loopTo1 = aHitbox.unused_VERSION49.Length - 1; x <= loopTo1; x++)
        ...
        public void ReadLocalAnimationDescs()
        {
            ...
            for (int i = 0, loopTo = theMdlFileData.localAnimationCount - 1; i <= loopTo; i++)
            {
                ...
                var anAnimationDesc = new SourceMdlAnimationDesc49();
        ...
        public void ReadAnimationSections()
        {
            if (theMdlFileData.theAnimationDescs is object)
            {
                foreach (SourceMdlAnimationDesc49 anAnimationDesc in theMdlFileData.theAnimationDescs)
                {
                    ...
                    SourceAniFrameAnim49 aSectionOfFrameAnimation;
                    ...
                    if (anAnimationDesc.sectionOffset != 0 && anAnimationDesc.sectionFrameCount > 0)
                    {
                        ...
                        int sectionCount = (int)Math.Round(Math.Truncate(anAnimationDesc.frameCount / (double)anAnimationDesc.sectionFrameCount)) + 2;

                        // NOTE: First sectionOfAnimation was created above.
                        for (int sectionIndex = 1, loopTo = sectionCount - 1; sectionIndex <= loopTo; sectionIndex++)
                        {
                            aSectionOfFrameAnimation = new SourceAniFrameAnim49();
                            anAnimationDesc.theSectionsOfFrameAnim.Add(aSectionOfFrameAnimation);
                            aSectionOfAnimation = new List<SourceMdlAnimation>();
                        ...
                        for (int sectionIndex = 0, loopTo1 = sectionCount - 1; sectionIndex <= loopTo1; sectionIndex++)
        ...
        protected void ReadAnimationFrameByBone(long animInputFileStreamPosition, SourceMdlAnimationDesc49 anAnimationDesc, int sectionFrameCount, int sectionIndex, bool lastSectionIsBeingRead)
        {
            ...
            fileOffsetStart = theInputFileReader.BaseStream.Position;
            SourceAniFrameAnim49 aSectionOfAnimation;
        ...
        protected void ReadMdlAnimation(long animInputFileStreamPosition, SourceMdlAnimationDesc49 anAnimationDesc, int sectionFrameCount, int sectionIndex, bool lastSectionIsBeingRead)
        {
            ...
            for (int j = 0, loopTo = boneCount - 1; j <= loopTo; j++)
            {
                ...
                theMdlFileData.theFileSeekLog.Add(animationInputFileStreamPosition, theInputFileReader.BaseStream.Position - 1L, "anAnimationDesc.anAnimation  [" + anAnimationDesc.theName + "] (frameCount = " + anAnimationDesc.frameCount.ToString() + "; sectionFrameCount = " + sectionFrameCount.ToString() + ")");
        ...
        private void ReadModels(long bodyPartInputFileStreamPosition, SourceMdlBodyPart aBodyPart, int bodyPartIndex)
        {
            if (aBodyPart.modelCount > 0)
            {
                ...
                for (int j = 0, loopTo = aBodyPart.modelCount - 1; j <= loopTo; j++)
                {
                    ...
                    theInputFileReader.BaseStream.Seek(inputFileStreamPosition, SeekOrigin.Begin);
                    modelName = Conversions.ToString(aModel.name).Trim('\0');
                    fileOffsetEnd = theInputFileReader.BaseStream.Position - 1L;
        ...
        public void PostProcess()
        {
            if (theMdlFileData.theBodyParts is object)
            {
                foreach (SourceMdlBodyPart aBodyPart in theMdlFileData.theBodyParts)
                {
                    ...
                    {
                        if (aBodyModel.theEyeballs is object && aBodyModel.theEyeballs.Count > 0)
                        {
        ...
        public void CreateFlexFrameList()
        {
            ...
            if (theMdlFileData.theFlexDescs is object && theMdlFileData.theFlexDescs.Count > 0)
            {
                ...
                int cumulativebodyPartVertexIndexStart;
                ...
                for (int bodyPartIndex = 0, loopTo = theMdlFileData.theBodyParts.Count - 1; bodyPartIndex <= loopTo; bodyPartIndex++)
                {
                    ...
                    for (int x = 0, loopTo1 = theMdlFileData.theFlexDescs.Count - 1; x <= loopTo1; x++)
                    {
                ...
                if (theMdlFileData.animBlockNameOffset > 0)
                {
                    if (theMdlFileData.version == 44)
                    {
                        ...
                        if (theMdlFileData.theAnimBlockRelativePathFileName.Length > internalAniFileName.Length)
                        {
                            theOutputFileWriter.BaseStream.Seek(0L, SeekOrigin.End);
                            long offset;
                            offset = theOutputFileWriter.BaseStream.Position;
