...
namespace Crowbar
{
    public class SourceSmdFile49
    {
        ...
        public void WriteNodesSection()
        {
            ...
            for (int boneIndex = 0, loopTo = boneCount - 1; boneIndex <= loopTo; boneIndex++)
            {
        ...
        public void WriteSkeletonSection(int lodIndex)
        {
            ...
            for (int boneIndex = 0, loopTo = theMdlFileData.theBones.Count - 1; boneIndex <= loopTo; boneIndex++)
            {
                ...
                // ======
                line += " ";
                line += aBone.position.x.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += aBone.position.y.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += aBone.position.z.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += aBone.rotation.x.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += aBone.rotation.y.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                line += " ";
                line += aBone.rotation.z.ToString("0.000000", Program.TheApp.InternalNumberFormat);
                // ------
        ...
        public void WriteSkeletonSectionForAnimation(SourceMdlSequenceDescBase aSequenceDescBase, SourceMdlAnimationDescBase anAnimationDescBase, bool onlyWriteCorrectiveAnimationRootBones = false)
        {
            ...
            var tempRotation = new SourceVector();
            SourceMdlSequenceDesc aSequenceDesc;
            SourceMdlAnimationDesc49 anAnimationDesc;
            bool thisIsForFirstSequence;
            var beforeMovementFrameRotation = new SourceVector();
            var previousFrameRotation = new SourceVector();
            var beforeMovementFramePosition = new SourceVector();
            var previousFramePosition = new SourceVector();
            aSequenceDesc = (SourceMdlSequenceDesc)aSequenceDescBase;
            anAnimationDesc = (SourceMdlAnimationDesc49)anAnimationDescBase;
            thisIsForFirstSequence = Conversions.ToString(anAnimationDesc.theName[0]) == "@" && anAnimationDesc.theAnimIsLinkedToSequence && ReferenceEquals(anAnimationDesc.theLinkedSequences[0], theMdlFileData.theSequenceDescs[0]);
            ...
            if (theMdlFileData.theBones is object)
            {
                ...
                for (int frameIndex = 0, loopTo = frameCount - 1; frameIndex <= loopTo; frameIndex++)
                {
                    ...
                    for (int i = 0, loopTo2 = theAnimationFrameLines.Count - 1; i <= loopTo2; i++)
                    {
                        ...
                        AdjustPositionAndRotation(boneIndex, adjustedPosition, adjustedRotation, thisIsForFirstSequence, ref position, ref rotation);
                        if (onlyWriteCorrectiveAnimationRootBones)
                        {
                            position.x = 0d;
                            position.y = 0d;
        ...
        private void AdjustPositionAndRotation(int boneIndex, SourceVector iPosition, SourceVector iRotation, bool thisIsForFirstSequence, ref SourceVector oPosition, ref SourceVector oRotation)
        {
            ...
            {
                if (thisIsForFirstSequence && theMdlFileData.theUpAxisYCommandWasUsed)
                {
                ...
                if (movements is object && frameIndex > 0)
                {
                    ...
                    foreach (SourceMdlMovement aMovement in movements)
                    {
                        if (frameIndex <= aMovement.endframeIndex)
                        {
                            ...
                            vecAngle.y = vecAngle.y * (1d - f) + aMovement.angle * f;
                            break;
                        }
                        else
                        {
        ...
        private SourceVector TransformPhyVertex(SourceMdlBone aBone, SourceVector vertex, SourcePhyPhysCollisionModel aSourcePhysCollisionModel)
        {
            ...
            else if (thePhyFileData.theSourcePhyIsCollisionModel)
            {
                ...
                if ((theMdlFileData.flags & SourceMdlFileData.STUDIOHDR_FLAGS_STATIC_PROP) > 0)
                {
                    ...
                    aVector.x = 1d / 0.0254d * vertex.z;
                    aVector.y = 1d / 0.0254d * -vertex.x;
                    aVector.z = 1d / 0.0254d * -vertex.y;
                ...
                else
                {
                    ...
                    aVector.x = 1d / 0.0254d * vertex.x;
                    aVector.y = 1d / 0.0254d * -vertex.y;
                    aVector.z = 1d / 0.0254d * -vertex.z;
                    aVectorTransformed = MathModule.VectorTransform(aVector, worldToPoseColumn0, worldToPoseColumn1, worldToPoseColumn2, worldToPoseColumn3);
                    aVector.x = aVectorTransformed.x;
                    aVector.y = aVectorTransformed.y;
                    aVector.z = aVectorTransformed.z;
