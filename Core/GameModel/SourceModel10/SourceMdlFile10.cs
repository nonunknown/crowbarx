            ...
            if (theMdlFileData.bodyPartCount > 0)
            {
                ...
                try
                {
                    ...
                    for (int bodyPartIndex = 0, loopTo = theMdlFileData.bodyPartCount - 1; bodyPartIndex <= loopTo; bodyPartIndex++)
                    {
                        ...
                        var aBodyPart = new SourceMdlBodyPart10();
                    ...
                    if (modelsEndInputFileStreamPosition > 0L)
                    {
                        inputFileStreamPosition = theInputFileReader.BaseStream.Position;
                        theMdlFileData.theFileSeekLog.LogToEndAndAlignToNextStart(theInputFileReader, modelsEndInputFileStreamPosition - 1L, 4, "theMdlFileData.theBodyParts - End of Models alignment");
                        theInputFileReader.BaseStream.Seek(inputFileStreamPosition, SeekOrigin.Begin);
                    }
                }
        ...
        private void ReadModels(SourceMdlBodyPart10 aBodyPart)
        {
            ...
            try
            {
                ...
                for (int bodyPartIndex = 0, loopTo = aBodyPart.modelCount - 1; bodyPartIndex <= loopTo; bodyPartIndex++)
                {
                    ...
                    var aModel = new SourceMdlModel10();
        ...
        private void ReadFaces(SourceMdlMesh10 aMesh)
        {
            if (aMesh.faceCount > 0)
            {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        var aStripOrFan = new SourceMeshTriangleStripOrFan10();
                        short listCount;
                        listCount = theInputFileReader.ReadInt16();
                        if (listCount == 0)
                        {
                        ...
                        listCount = Math.Abs(listCount);
                        for (int listIndex = 0, loopTo = listCount - 1; listIndex <= loopTo; listIndex++)
                        {
                            var vertexAndNormalIndexInfo = new SourceMdlVertexInfo10();
