    ...
    public class SourceSmdFile10
    {
            ...
            try
            {
                if (aBodyModel.theMeshes is object)
                {
                    for (int meshIndex = 0, loopTo = aBodyModel.theMeshes.Count - 1; meshIndex <= loopTo; meshIndex++)
                    {
                        ...
                        if (aMesh.theStripsAndFans is object)
                        {
                            for (int stripsAndFansIndex = 0, loopTo1 = aMesh.theStripsAndFans.Count - 1; stripsAndFansIndex <= loopTo1; stripsAndFansIndex++)
                            {
                                SourceMeshTriangleStripOrFan10 aStripOrFan;
                                aStripOrFan = aMesh.theStripsAndFans[stripsAndFansIndex];
                                if (aStripOrFan.theFacesAreStoredAsTriangleStrips)
                                {
                                    for (int vertexInfoIndex = 0, loopTo2 = aStripOrFan.theVertexInfos.Count - 3; vertexInfoIndex <= loopTo2; vertexInfoIndex++)
                                    {
                                ...
                                {
                                    for (int groupIndex = 1, loopTo3 = aStripOrFan.theVertexInfos.Count - 2; groupIndex <= loopTo3; groupIndex++)
                                    {
                                        materialLine = materialName;
        ...
        private string GetVertexLine(SourceMdlModel10 aBodyModel, SourceMdlVertexInfo10 aVertexInfo, SourceMdlTexture10 aTexture)
        {
            ...
            try
            {
                ...
                SourceBoneTransform10 boneTransform;
                ...
                if (aTexture.width == 1L || aTexture.height == 1L)
                {
                    if (Conversions.ToString(aTexture.theFileName[0]) == "#")
                    {
                        uint width;
                        uint height;
                        width = Conversions.ToUInteger(aTexture.theFileName.Substring(1, 3));
                        height = Conversions.ToUInteger(aTexture.theFileName.Substring(4, 3));
                ...
                else if (Program.TheApp.Settings.DecompileUseNonValveUvConversionIsChecked)
                {
                ...
                {
                    texCoordX = aVertexInfo.s / (double)(aTexture.width - 1L);
                    texCoordY = aVertexInfo.t / (double)(aTexture.height - 1L);
                }
                ...
                line += (1d - texCoordY).ToString("0.000000", Program.TheApp.InternalNumberFormat);
            }
            catch (Exception ex)
            {
                line = "// " + line;
