...
namespace Crowbar
{
    static class MathModule
    {
        ...
        {
            var dtDateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
            var span = iDateTime.ToUniversalTime() - dtDateTime;
        ...
        public static string ByteUnitsConversion(ulong iBytes)
        {
            double value = iBytes;
            string suffix = "Bytes";
            if (iBytes >= 1073741824m)
            {
                value = iBytes / (double)(1024 * 1024 * 1024);
                suffix = "GB";
            }
            else if (iBytes >= 1048576m)
            {
                value = iBytes / (double)(1024 * 1024);
                suffix = "MB";
            }
            else if (iBytes >= 1024m)
            {
                value = iBytes / 1024d;
                suffix = "KB";
            }
        ...
        public static long AlignLong(long currentValue, long alignmentValue)
        {
            ...
            long result;
            result = currentValue + alignmentValue - 1L & ~(alignmentValue - 1L);
            return result;
        }
        ...
        public static double DegreesToRadians(double degrees)
        {
            ...
            return degrees * 3.1415926535897931d / 180d;
        }
        ...
        public static double RadiansToDegrees(double radians)
        {
            ...
            return radians * 180d / 3.1415926535897931d;
        }
        ...
        public static SourceQuaternion EulerAnglesToQuaternion(SourceVector angleVector)
        {
            ...
            fYaw = angleVector.y;
            fRoll = angleVector.z;
            double fSinPitch = Math.Sin(fPitch * 0.5d);
            double fCosPitch = Math.Cos(fPitch * 0.5d);
            double fSinYaw = Math.Sin(fYaw * 0.5d);
            double fCosYaw = Math.Cos(fYaw * 0.5d);
            double fSinRoll = Math.Sin(fRoll * 0.5d);
            double fCosRoll = Math.Cos(fRoll * 0.5d);
        ...
        {
            var forward = new SourceVector();
            var left = new SourceVector();
            var up = new SourceVector();
        ...
        {
            SourceVector output;
            SourceVector matrixRow0;
            SourceVector matrixRow1;
            SourceVector matrixRow2;
        ...
        {
            var M = new double[,] { { 0d, 0d, 0d, 0d }, { 0d, 0d, 0d, 0d }, { 0d, 0d, 0d, 0d }, { 0d, 0d, 0d, 0d } };
            double Nq;
            double s;
            double xs;
            double ys;
        ...
        private const double FLT_EPSILON = 0.00001d;
    }
}