...
namespace Crowbar
{
    public class Decompiler : BackgroundWorker
    {
        ...
        {
            string pathFileName;
            pathFileName = Path.Combine(theOutputPath, relativePathFileName);
        ...
        {
            string outputPath;
            if (Program.TheApp.Settings.DecompileOutputFolderOption == AppEnums.DecompileOutputPathOptions.Subfolder)
            {
                if (File.Exists(Program.TheApp.Settings.DecompileMdlPathFileName))
                {
                    outputPath = Path.Combine(FileManager.GetPath(Program.TheApp.Settings.DecompileMdlPathFileName), Program.TheApp.Settings.DecompileOutputSubfolderName);
                }
                else if (Directory.Exists(Program.TheApp.Settings.DecompileMdlPathFileName))
                {
                    outputPath = Path.GetFullPath(Path.Combine(Program.TheApp.Settings.DecompileMdlPathFileName, Program.TheApp.Settings.DecompileOutputSubfolderName));
                }
                else
                {
            ...
            {
                outputPath = Program.TheApp.Settings.DecompileOutputFullPath;
            }
        ...
        {
            bool inputsAreValid;
            if (string.IsNullOrEmpty(Program.TheApp.Settings.DecompileMdlPathFileName))
            {
        ...
        public AppEnums.StatusMessage Decompile(string mdlInput)
        {
            ...
            string mdlPathFileName;
            mdlPathFileName = mdlInput;
            ...
            progressDescriptionText = "Decompiling with " + Program.TheApp.GetProductNameAndVersion() + ": ";
            try
            {
                ...
                // else if (Program.TheApp.Settings.DecompileMode == AppEnums.InputOptions.FolderRecursion)
                // {
                ...
                // }
                // else if (Program.TheApp.Settings.DecompileMode == AppEnums.InputOptions.Folder)
                // {
        ...
        private AppEnums.StatusMessage DecompileOneModel(string mdlPathFileName)
        {
            ...
            {
                string mdlFileName;
                string mdlRelativePathName;
                string mdlRelativePathFileName;
                ...
                theModelOutputPath = Program.TheApp.decompiledOutputPath;//"/home/nonunknown/Desktop/test/";//TODO;//Path.Combine(theOutputPath, mdlRelativePathName);
                // theModelOutputPath = Path.GetFullPath(theModelOutputPath);
                // if (Program.TheApp.Settings.DecompileFolderForEachModelIsChecked)
                // {
                ...
                // Return status
                // End Try
                //if (Program.TheApp.Settings.DecompileMode == AppEnums.InputOptions.File)
                // if (true)
                // {
                ...
                try
                {
                    model = SourceModel.Create(mdlPathFileName, AppEnums.SupportedMdlVersion.DoNotOverride, ref version);
                    if (model is object)
                    {
                        // if (Program.TheApp.Settings.DecompileOverrideMdlVersion == AppEnums.SupportedMdlVersion.DoNotOverride)
                        // {
                ...
                AppEnums.FilesFoundFlags filesFoundFlags;
                ...
                if ((int)(filesFoundFlags & AppEnums.FilesFoundFlags.ErrorRequiredAniFileNotFound) > 0)
                {
                    UpdateProgress(3, "WARNING: ANI file not found.");
                }

                if ((int)(filesFoundFlags & AppEnums.FilesFoundFlags.ErrorRequiredVtxFileNotFound) > 0)
                {
                    UpdateProgress(3, "WARNING: VTX file not found.");
                }

                if ((int)(filesFoundFlags & AppEnums.FilesFoundFlags.ErrorRequiredVvdFileNotFound) > 0)
                {
            ...
            {
                Console.WriteLine("Error at compiling the Model: " + ex.Message);
                // int debug = 4242;
                // Finally
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileLogFileIsChecked)
            {
                string mdlFileName;
                string logPath;
                string logFileName;
                ...
                try
                {
                    ...
                    theLogFileStream.WriteLine("// " + Program.TheApp.GetHeaderComment());
                    theLogFileStream.Flush();
                }
                catch (Exception ex)
                {
        ...
        private AppEnums.StatusMessage ReadCompiledFiles(string mdlPathFileName, SourceModel model)
        {
            ...
            if (model.AniFileIsUsed && Program.TheApp.Settings.DecompileBoneAnimationSmdFilesIsChecked)
            {
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            Program.TheApp.SmdFileNames.Clear();
        ...
        private AppEnums.StatusMessage WriteQcFile(SourceModel model)
        {
            ...
            if (true)
            {
                ...
                qcPathFileName = Path.Combine(theModelOutputPath, model.Name + ".qc");
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileReferenceMeshSmdFileIsChecked)
            {
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileLodMeshSmdFilesIsChecked)
            {
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompilePhysicsMeshSmdFileIsChecked)
            {
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileVertexAnimationVtaFileIsChecked)
            {
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileBoneAnimationSmdFilesIsChecked)
            {
                if (model.HasBoneAnimationData)
                {
                    string outputPath;
                    outputPath = Path.Combine(theModelOutputPath, SourceFileNamesModule.GetAnimationSmdRelativePath(model.Name));
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileProceduralBonesVrdFileIsChecked)
            {
                if (model.HasProceduralBonesData)
                {
                    ...
                    theFirstDecompiledFileHasBeenAdded = false;
                    model.SourceModelProgress += Model_SourceModelProgress;
                    string vrdPathFileName;
                    vrdPathFileName = Path.Combine(theModelOutputPath, SourceFileNamesModule.GetVrdFileName(model.Name));
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileDeclareSequenceQciFileIsChecked)
            {
                if (model.HasBoneAnimationData)
                {
                    ...
                    theFirstDecompiledFileHasBeenAdded = false;
                    model.SourceModelProgress += Model_SourceModelProgress;
                    string declareSequenceQciPathFileName;
                    declareSequenceQciPathFileName = Path.Combine(theModelOutputPath, SourceFileNamesModule.GetDeclareSequenceQciFileName(model.Name));
        ...
        {
            var status = AppEnums.StatusMessage.Success;
            if (Program.TheApp.Settings.DecompileTextureBmpFilesIsChecked)
            {
        ...
        {
            string debugPath;
            debugPath = Program.TheApp.GetDebugPath(theModelOutputPath, model.Name);
        ...
        {
            UpdateProgressInternal(100, Constants.vbCr + line);
        }
        ...
        {
            if (progressValue == 1 && theLogFileStream is object)
            {
                theLogFileStream.WriteLine(line);
                theLogFileStream.Flush();
            }
            Console.WriteLine(progressValue.ToString()+"% "+ line);
        ...
        private void Model_SourceModelProgress(object sender, SourceModelProgressEventArgs e)
        {
            ...
            else if (e.Progress == AppEnums.ProgressOptions.WritingFileFinished)
            {
                ...
                if (!theFirstDecompiledFileHasBeenAdded && File.Exists(pathFileName))
                {
                    ...
                    relativePathFileName = FileManager.GetRelativePathFileName(theOutputPath, pathFileName);
